            ...
            <div className="sticky top-32 botanical-card p-8 md:p-10 border-t-4 border-t-[#6B8060] overflow-hidden relative">
              ...
              <div className="space-y-6 mb-10">
                ...
                <motion.div 
                  ...
                  className={`border rounded-3xl p-6 relative overflow-hidden transition-all duration-500 ${saved > 0 ? 'border-emerald-500/40 shadow-lg shadow-emerald-500/10' : 'border-[#6B8060]/20'}`}
                >
                  {saved > 0 && (
                    <div className="absolute top-0 right-0 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-wider py-1.5 px-4 rounded-bl-2xl shadow-sm flex items-center gap-1">
                      <TrendingDown size={14} strokeWidth={3} /> Save {Math.round((saved / formData.amount) * 100)}%
                    </div>
                  )}
                ...
                {formData.amount > 0 && (
                  <div className="space-y-3 px-2">
                    <div className="h-4 w-full bg-[#EAE3D2] rounded-full overflow-hidden flex shadow-inner border border-[#6B8060]/10">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${Math.max(0, ((formData.amount - saved) / formData.amount) * 100)}%` }} transition={{ duration: 1, ease: "easeOut" }} className="h-full bg-[#1A1F16]" />
                      <motion.div initial={{ width: 0 }} animate={{ width: `${(saved / formData.amount) * 100}%` }} transition={{ duration: 1, ease: "easeOut" }} className="h-full bg-emerald-500 relative">
